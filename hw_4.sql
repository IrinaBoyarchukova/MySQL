...
SELECT 
  ...
  (SELECT CONCAT (firstname, ' ', lastname) FROM users WHERE id = likes.user_id) AS 'Имя_Фамилия',
...
Where 
  (SELECT TIMESTAMPDIFF(YEAR, birthday, NOW()) FROM profiles WHERE user_id = likes.user_id) <12
GROUP BY 
  user_id;
...
-- 2. Определить кто больше поставил лайков(всего): мужчины или женщины.
 ...
 SELECT 
   profiles.gender  AS 'Пол',
   COUNT(*) AS 'Количество лайков'
FROM 
  likes 
  Left join profiles on profiles.user_id = likes.user_id
GROUP BY 
  profiles.gender ;
----------------------------------
...
SELECT *
From profiles 
Left join messages on messages.from_user_id = profiles.user_id
...
select Count( messages.id) , messages.from_user_id 
from (SELECT initiator_user_id as frends
	FROM friend_requests 
	WHERE target_user_id = 1 AND status = 'approved'
	UNION 
	SELECT target_user_id 
	FROM friend_requests 
	WHERE initiator_user_id = 1 AND status = 'approved') as t 
	Left join messages on messages.from_user_id = T.frends and messages.to_user_id = 1
